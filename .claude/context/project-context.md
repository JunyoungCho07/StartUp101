...
## 전문가 인터뷰 인사이트 (유통 임원 30년, KAIST 제출 답변)

> 출처: `customer-discovery/interviews/transcripts/expert_interview_retail_executive_kaist_utf8.txt`
> 프로파일: 까르푸·월마트·이마트·쿠팡·롯데쇼핑 역임, 리테일 Value Chain 전반

### 설문 데이터와 교차 검증된 Pain (전문가 확인)

| Pain | 설문 데이터 | 전문가 발언 |
|---|---|---|
| 셀프계산대 직원 호출 | 33% 경험 | "스캔 오류·성인인증·쿠폰 시 직원 개입 빈번" |
| 대기열 병목 | 피크 타임 7명(15%) 강한 Pain | "주말 피크타임 셀프 대기열도 길어진다" |
| 디지털 소외 | 직접 측정 없음 | "고연령층에게 오히려 진입 장벽" — 새 gap |

### B2B 전략에 직접 적용 가능한 인사이트

**① 도입 의사결정 3축 (경영진 설득 프레임)**
- ROI: BEP 2년 내 → NoCall 파일럿 ROI 시뮬레이션 준비 필수
- SLA: 즉각 AS 체계 명시 필요
- 시스템 통합: POS·재고·멤버십 API 연동 로드맵 필요

**② 스마트카트 실패 교훈 — End-to-End 경험 완결**
- 초기 모델 실패 원인: 고가 기기 → 매장 외 반출 금지 → 주차장까지 짐 직접 운반
- NoCall 설계 원칙: 계산 완료 후 주차장 도달 순간까지 경험 설계 필요

**③ B2B 계약 모델 (전문가 직접 권장)**
- 전액 매입 모델: 거부 확실 ("어느 회사도 수용 안 한다")
- 선호 모델: **카트 1대당 월 구독료** (하드웨어 감가 + SW 업데이트 + AS 포함)
- 이유: CapEx → OpEx 전환으로 의사결정 허들 감소

**④ 파일럿 설계 KPI (전문가 제시)**
- 스캔 오류율 < **0.1%** (일 5,000명 기준 5명 손실 = 치명적)
- 카트 1대당 TCO(총소유비용) 정당화 데이터
- 내구성·배터리 테스트 결과서
